...
window.onload = function () {
    //getting url params (if any) to check if this is index or starship page
...
function indexPage() {
    ...
    if (filmsJson !== false) {
        ...
        for (const film of films.results) {
            //add movies to <ul>
            movieList.innerHTML += '<li><div class="movie"><div>' + film.title + ' <span class="episode">' + film.episode_id + '</span> <i>' + film.release_date + '</i></div><a href="starship.html?id=' + id + '">Starships</a></div></li>';
            id++;
        }
    }
}
...
function starshipPage(id) {
    ...
    if (filmJson !== false) {
        ...
        paginationElement.innerHTML = '<button onclick="prevPage()">&lt</button>' + html + '<button onclick="nextPage()">&gt</button>';
    }
}
